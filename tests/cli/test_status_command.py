...
def test_status_command_running_task(
    ...
    type(running_session).duration = mock.PropertyMock(return_value=expected_duration)
...
def test_status_command_paused_task(
    ...
    # For a paused session, duration returns _accumulated_duration
    type(paused_session).duration = mock.PropertyMock(
        return_value=paused_session._accumulated_duration
    )
...
def test_status_command_multiple_active_error(
    ...
    mock_storage_provider_status.get_all_sessions.return_value = [session1, session2]
    mock_json_storage_class.return_value = mock_storage_provider_status
